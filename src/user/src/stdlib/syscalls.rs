...
pub fn exit(exit_code: i64) -> ! {
    ...
    if let Ok((pid, ppid)) = get_p_info() && crate::INFO_PRINTS_ENABLED {
        crate::println!("process with PID {} from parent PID {}, exiting with exit code {}", pid, ppid, exit_code);
    }
    ...
    if let Ok((pid, ppid)) = get_p_info() && crate::INFO_PRINTS_ENABLED {
        crate::println!("process failed to exit! pid: {}, parent pid: {}, exit code: {}", pid, ppid, exit_code);
    }
...
pub fn fork() -> Result<u64, &'static str> {
    ...
    if crate::INFO_PRINTS_ENABLED && r == 0 && let Ok((pid, ppid)) = get_p_info() {
        crate::println!("new process spawned with PID: {}, from parent PID: {}", pid, ppid);
    }
